...
import { EnvironmentInjector, Injectable, InjectionToken, ɵPendingTasks as PendingTasks, inject, runInInjectionContext } from '@angular/core';
...
export const HTTP_INTERCEPTORS =
    new InjectionToken<readonly HttpInterceptor[]>('');

/**
 * A multi-provided token of `HttpInterceptorFn`s.
 */
export const HTTP_INTERCEPTOR_FNS =
    new InjectionToken<readonly HttpInterceptorFn[]>('');
...
export const PRIMARY_HTTP_BACKEND =
    new InjectionToken<HttpBackend>('');
...
export class HttpInterceptorHandler extends HttpHandler {
  ...
  override handle(initialRequest: HttpRequest<any>): Observable<HttpEvent<any>> {
    ...
    if (this.chain === null) {
      ...
      // In this way we are going to convert set to array
      ...
      // this is a simple reducer that loop from last to first
